...
export default function NavLink(props: NavLinkProps): JSX.Element {
  ...
  return (
    <li className='' style={{ width: `${width * 100}%` }}>
      <Link className='text-primary-100 flex items-center justify-center h-full gap-2 p-2 transition-colors hover:text-secondary-400' href={`#${props.id}`}>{props.children}</Link>
    </li>
  );
}