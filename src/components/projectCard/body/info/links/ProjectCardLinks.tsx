...
export default function ProjectCardLinks(props: ProjectCardLinksProps): JSX.Element {
  ...
  return (
    <div className={utils.cn(`flex flex-row justify-around items-center w-full bg-primary-2 text-primary-3 ${showReadMore ? 'h-0 text-sm' : 'h-fit p-2'}`, styles?.links)}>
      ...
      <a role='link' className='underline hocus:text-primary-0' href={project.source} target='_blank' rel='noopener noreferrer'>src code</a>
      <a role='link' className='underline hocus:text-primary-0' href={project.site} target='_blank' rel='noopener noreferrer'>visit site</a>
    </div>
  );
}